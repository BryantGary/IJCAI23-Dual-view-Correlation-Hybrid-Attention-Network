...
class CNN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        #self.conv4 = nn.Conv2d(2048, 2048, kernel_size=1, stride=1, padding=0,
                               #bias=False)                       
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        #if(planes==512):
          #layers.append(block(self.inplanes, planes, stride, downsample,groups=2))
        #else:
        layers.append(block(self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
          
          if (planes==512 and i==2):#Position for injecting LR layer 
    ...
    def forward(self, x):
       ...
       # x = self.local1(x)
        ...
        #for i in range(x.size(3)):
         #x[:,:,:,i] = self.net1(x[:,:,:,i])
        #x= self.conv4(x)  
        x_feature = x  #Feature for correlation
        x = self.avgpool1(x)
